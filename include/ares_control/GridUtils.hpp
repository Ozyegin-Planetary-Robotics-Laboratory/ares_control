...
bool GridUtils::isCollisionCourse(const nav_msgs::Path &path) {
...
size_t GridUtils::getIndex_(const geometry_msgs::PoseStamped &pose) {
  Point p = getIndices_(pose);
  return getIndex_(p[0], p[1]);
}

size_t GridUtils::getIndex_(const int &i1, const int &i2) {
  return i2 * obstacle_grid->info.width + i1;
}

GridUtils::Point GridUtils::getIndices_(const geometry_msgs::PoseStamped &pose) {
  Point p;
...
GridUtils::Line GridUtils::getLine_(const geometry_msgs::PoseStamped &pose1, const geometry_msgs::PoseStamped &pose2) {
  Line line;
...
GridUtils::Line GridUtils::getLine_(const Point &p1, const Point &p2) {
  Line line;
  int x1 = p1[0];
  int y1 = p1[1];
  int x2 = p2[0];
...
bool GridUtils::isBlocked_(const Line &line) {
